...
export function CorporateBenefitsForm({ compact = false, variant = "consultation" }: { compact?: boolean; variant?: "consultation" | "meeting" }) {
  const [form, setForm] = useState({ firstName: "", lastName: "", email: "", phone: "", company: "", employees: "", role: "", preferredTime: "", consent: false });
  ...
  async function submit(event: React.FormEvent) {
    ...
    const result = await submitLead({
      ...
      smsConsent: form.consent, source: "corporate-benefits", funnelType: variant === "meeting" ? "corporate-benefits-meeting" : "corporate-home-benefits",
      notes: [`Company: ${form.company}`, `Role: ${form.role}`, `Employee count: ${form.employees}`, variant === "meeting" ? `Preferred meeting time: ${form.preferredTime}` : ""].filter(Boolean).join("\n"),
    });
  ...
  return (
    <form onSubmit={submit} className={compact ? "space-y-3" : "space-y-4"}>
      ...
      {variant === "meeting" && <textarea required className={`${input} h-24 py-3`} placeholder="Preferred days, times, and time zone" value={form.preferredTime} onChange={e=>setForm({...form,preferredTime:e.target.value})}/>}
      ...
      <button disabled={status === "loading" || !form.consent} className="primary-button w-full disabled:opacity-50">{status === "loading" ? "Sending…" : variant === "meeting" ? "Request my meeting →" : "Request a program consultation →"}</button>
    </form>
  );
}
