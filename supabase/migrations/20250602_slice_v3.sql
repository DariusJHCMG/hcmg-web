-- ═══════════════════════════════════════════════════════════════
-- SLICE by HCMG — v3 Migration (additive, safe to apply over v2)
...
-- ── 1. EXTENDED PROFILES ────────────────────────────────────────
-- Add new columns to profiles (safe: all nullable / have defaults)
...
create table if not exists public.harry_ai_insights (
  ...
  reporting_period    text,           -- e.g. "2025-07"
  goal_month_id       uuid references public.goal_months(id) on delete set null,
...
-- ── 12. COMMITMENT REMINDER LOG (idempotency) ───────────────────
...
-- ── 13. SLICE SESSION TOKENS (custom auth) ──────────────────────
