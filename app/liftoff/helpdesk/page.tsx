...
function demoBase(overrides: Partial<LiftOffRequest>): LiftOffRequest {
  return {
    ...
    request_type: "loan_help_desk", request_status: "pending",
...
const DEMO_VIEWER_ID   = "demo-hd-1";
const DEMO_VIEWER_NAME = "Jordan Lee";

const DEMO_REQUESTS: LiftOffRequest[] = [
  demoBase({
    id: "demo-hd-pending-1",
    ...
    help_desk_sub_type: "suspense_conditions",
    help_desk_description: "File hit suspense — UW is requesting 12 months bank statements but borrower only has 3 months available. Need guidance on how to proceed and whether a letter of explanation is sufficient for the remaining months.",
    created_at: mins(15),
    updated_at: mins(15),
    certified_at: mins(15),
    priority_score: 80,
  }),
  demoBase({
    id: "demo-hd-inreview-1",
    ...
    help_desk_sub_type: "income_asset_question",
    help_desk_description: "Borrower is self-employed with an S-Corp. UW is questioning the distribution income vs. salary. Need help structuring the explanation letter and confirming which income to use for qualifying.",
    created_at: mins(90),
    updated_at: mins(40),
    certified_at: mins(90),
    claimed_by_id: DEMO_VIEWER_ID,
    claimed_by_name: DEMO_VIEWER_NAME,
    claimed_at: mins(40),
    started_at: mins(20),
    inflight_email_sent_at: mins(20),
    priority_score: 70,
  }),
  demoBase({
    id: "demo-hd-action-1",
    ...
    help_desk_description: "AUS ran DU and got Refer/Eligible. LO wants to know if we can run LP instead and if there is a better outcome expected with the borrower's profile.",
    created_at: mins(240),
    updated_at: mins(60),
    certified_at: mins(240),
    claimed_by_id: "demo-hd-2",
    claimed_by_name: "Alex Nguyen",
    claimed_at: mins(180),
    started_at: mins(150),
    incomplete_reasons: ["Issue description too vague — please provide more detail", "ARIVE loan number not found or not provided"],
    incomplete_notes: "Need the full DU findings report attached in ARIVE before we can advise.",
    incomplete_at: mins(60),
    incomplete_by_name: "Alex Nguyen",
    priority_score: 90,
  }),
  demoBase({
    id: "demo-hd-completed-1",
    ...
    help_desk_sub_type: "exception_request",
    help_desk_description: "Requesting an exception for a 2-unit investment property with 20% down. LTV is at guideline max and borrower has 740 credit. Need ops to review and approve exception before we proceed.",
    created_at: mins(480),
    updated_at: mins(10),
    certified_at: mins(480),
    claimed_by_id: DEMO_VIEWER_ID,
    claimed_by_name: DEMO_VIEWER_NAME,
    claimed_at: mins(420),
    started_at: mins(390),
    completed_at: mins(10),
    completed_email_sent_at: mins(10),
    priority_score: 60,
  }),
];
...
async function getHelpDeskRequests(): Promise<LiftOffRequest[]> {
  const sb = createServiceClient();
  const { data } = await sb
    .from("lift_off_requests")
    .select("*")
    .eq("request_type", "loan_help_desk")
...
export default async function HelpDeskQueuePage({
  ...
  if (!profile) redirect("/login?next=/liftoff/helpdesk");
    ...
    <div className="space-y-6">
      ...
      {isDemo && (
        <div className="rounded-2xl border-2 border-purple-300 bg-purple-50 px-6 py-4 flex items-center gap-3">
          ...
          <div>
            <p className="font-bold text-purple-800 text-sm">Demo Mode — Help Desk Queue</p>
            <p className="text-xs text-purple-700 mt-0.5">
              All actions (Claim, Start, Complete) work in this demo — no database writes or emails will be sent.
              Showing 4 realistic help desk requests across all stages.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-extrabold text-ink">Help Desk Queue</h1>
