 ...
 * Implemented as a Route Handler (not a page component) so it can
 * return raw HTML with Content-Type text/html.
 */
...
export async function GET(
  ...
  // Inject a sticky print bar above the certificate
  const printBar = `
    <div style="position:fixed;top:0;left:0;right:0;z-index:999;background:#142850;padding:10px 20px;display:flex;align-items:center;justify-content:space-between;font-family:'Helvetica Neue',sans-serif;box-shadow:0 2px 12px rgba(0,0,0,0.4);">
      <div style="display:flex;align-items:center;gap:12px;">
        ...
        <div>
          ...
          <p style="margin:0;font-size:11px;color:rgba(255,255,255,0.55);">${recipientName} &middot; ${monthLabel}</p>
        </div>
      </div>
