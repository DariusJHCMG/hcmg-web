    ...
    <div className="space-y-5">
      ...
      <div className="overflow-hidden rounded-2xl border border-violet-200 bg-violet-50">
        ...
        <div className="overflow-x-auto"><table className="w-full text-sm">
          ...
          <tbody>{leads.length === 0 && <tr><td colSpan={7} className="px-6 py-12 text-center text-muted">No Corporate Home Benefits inquiries yet.</td></tr>}{leads.map(lead => <LeadIntelPanel key={lead.id} lead={lead} sourceLabel="Corporate Benefits" hideLoColumn />)}</tbody>
        </table></div>
      </div>
    </div>
  );
