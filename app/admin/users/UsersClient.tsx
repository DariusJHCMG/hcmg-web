...
export function UsersClient({ initialUsers }: { initialUsers: Profile[] }) {
  ...
  const inputClass = "rounded-xl border border-line bg-white px-3 py-2.5 text-sm text-ink placeholder:text-muted/50 focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/20 transition";
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted">
          {users.length} accounts · Create and manage portal login accounts for your team.
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-line bg-white p-6">
        ...
        <form onSubmit={createUser} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="lg:col-span-2">
            ...
            <input required className={inputClass + " w-full"} placeholder="Lamont Harris Jr."
          ...
          <div>
            ...
            <select required className={inputClass + " w-full"} value={form.role}
          ...
          <div>
            ...
            <input required type="email" className={inputClass + " w-full"} placeholder="email@hcmgloans.com"
              value={form.email} onChange={(e) => setForm(p => ({ ...p, email: e.target.value }))} />
          </div>
          <div>
            <label className="mb-1 block text-xs font-semibold text-ink">Password *</label>
            <input required type="password" className={inputClass + " w-full"} placeholder="Min 8 chars"
              value={form.password} onChange={(e) => setForm(p => ({ ...p, password: e.target.value }))} />
          </div>
          <div>
            <label className="mb-1 block text-xs font-semibold text-ink">LO Slug (if LO)</label>
            <input className={inputClass + " w-full"} placeholder="lamont-harris-jr"
              value={form.lo_slug} onChange={(e) => setForm(p => ({ ...p, lo_slug: e.target.value }))} />
          </div>
          <div>
            <label className="mb-1 block text-xs font-semibold text-ink">NMLS#</label>
            <input className={inputClass + " w-full"} placeholder="1234567"
              value={form.nmls} onChange={(e) => setForm(p => ({ ...p, nmls: e.target.value }))} />
          </div>
          <div>
            <label className="mb-1 block text-xs font-semibold text-ink">Lead Notify Email</label>
            <input type="email" className={inputClass + " w-full"} placeholder="Defaults to login email"
          ...
          <div className="sm:col-span-2 lg:col-span-4 flex items-center gap-4">
            ...
            {msg && (
              <p className={`text-sm font-semibold ${msg.type === "ok" ? "text-green-600" : "text-red-600"}`}>
                {msg.text}
              </p>
            )}
          </div>
      ...
      <div className="rounded-2xl border border-line bg-white overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {users.length === 0 && (
                <tr><td colSpan={7} className="px-6 py-8 text-center text-sm text-muted/60">No users found.</td></tr>
              )}
              {users.map((u, i) => (
